...
function velocidad_instantanea() {
  let i = 0;
  const expression = document.getElementById("eq").value;
  const tiempo = Math.ceil(
    parseFloat(document.getElementById("testudio").value)
  );
  const tiempofinal = Math.ceil(
    parseFloat(document.getElementById("tfinal").value)
  );
  ...
  if (TieneLetras(expression) == true) {
    ...
    while (i <= tiempofinal) {
      ...
      if (i == tiempo) {
        respuesta.innerHTML =
          "La velocidad Instantanea en el segundo: " +
          i +
          " Es de: " +
          Resultado.toFixed(2) +
          " m/s";
    ...
    respuesta.innerHTML =
      "La velocidad del objeto es constante ya que no depende del tiempo";
  }
  return velocidades;
}
...
function recorrido() {
  (function () {
    ...
    $(function () {
      window.__context = {
        glowscript_container: $("#SimulacionVI").removeAttr("id"),
      };
      __main__();
    });
  })();
